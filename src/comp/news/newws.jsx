  ...
  return (
    <>
      <div>
        ...
        <Typography variant='h3' sx={{ paddingTop: { xs: 8, md: 3 } ,paddingLeft:{xs:1}}} fontWeight={600} alignItems={'center'} >
        ...
        <Stack direction={'row'} flexWrap={'wrap'} gap={6} justifyContent={'center'} p={2}>
          ...
          {Info && Info.map((ele) => (
            ...
            <Card key={ele.id} sx={{ width: { xs: 500, sm: 650, md: 500, lg: 500 } }} >


              <Stack sx={{ flexDirection: { xs: 'column', md: 'row', sm: 'row' }, textAlign: { xs: 'left', sm: 'left', md: 'left' } }} justifyContent={'space-between'} p={1} gap={2} alignItems={'center'} key={ele.Id} >
                <h2>{ele.name}</h2>
                <img src={ele.image?.thumbnail?.contentUrl} alt="" width={'100px'} />
              </Stack>
